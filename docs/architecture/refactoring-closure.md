...
The applied baseline is Refactor 35: core
`ebd99623f94c8b2db2d74355bfa39f01550778a0` and examples
`68a0bbd0e354b9e00bbfed728ad2b769b389ed8a` on `refactor/architecture`.
W1/W2, Storage S1 and S2a/S2b/S2c are applied and verified. Refactor 36 includes
S3's final internal ownership cleanup and current-document reconciliation.
Its distribution must still be applied and verified before claiming the user's
repository contains all completed work.

This completes the S3 audit and its bounded Storage cleanup candidate, not all
initial review proposals. The previous remaining-work summary omitted still
...
public behavior changes. First resume the small, previously delivered D02 after
Refactor 36 application verification. Then address R03/R11's shared vocabulary
...
Refactor 36 must be validated as its own candidate. Unpublished candidate CI and
live PostgreSQL cannot be inferred from Refactor 35's green checks. The final
application review should identify both the tested commit and source tree.
The published SVG remains on applied35-01 until that review, even though the
candidate removes a two-file constant-reference cycle inside Storage.

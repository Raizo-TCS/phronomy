...
The shared Runtime-internal FSMSession graph metadata bridge is unchanged by
this migration.
...
The Runtime routing half is intentionally unchanged in this migration slice.
`AgentInvocation#id`, `ToolInvocation#id`, and the private
`parent_agent_invocation_id` routing carrier remain temporary implementation
details until CG-03b / ACS-10 / ACS-11 establishes fresh FSMSession IDs and
session-local routing bindings.

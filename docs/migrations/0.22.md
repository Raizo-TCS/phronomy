...
This slice does not rename or remove the shared Runtime-internal FSMSession
graph metadata bridge. It also does not remove the durable
`JournalRecord#correlation_id` schema field. New Agent execution paths stop
supplying a generic correlation value; canonical Journal field removal and
legacy-record backward read are completed separately in CG-02b.

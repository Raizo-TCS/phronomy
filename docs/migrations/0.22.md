...
to the owning FSMSession before it updates Activation LLM-result state.

ACS-11 EventLoop single-writer / `AgentExecutionActivation` removal remains the
immediately following Runtime-foundation slice, and CG-03 remains open until the
joint foundation closes the remaining worker-side stale-result authority gap.
Workflow opaque admission ownership remains separate ACS-13 work.

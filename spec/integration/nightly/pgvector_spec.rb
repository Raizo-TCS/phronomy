...
RSpec.describe "Nightly: VectorStore::Pgvector against real PostgreSQL", :nightly, real_backend: :pgvector do
  ...
  let(:store) { Phronomy::Agent::Context::Knowledge::VectorStore::Pgvector.new(model_class: NightlyDoc, dimension: 3) }
  ...
  it_behaves_like "a vector store" do
    ...
    let(:store) { Phronomy::Agent::Context::Knowledge::VectorStore::Pgvector.new(model_class: NightlyDoc, dimension: 3) }
    let(:empty_store) { Phronomy::Agent::Context::Knowledge::VectorStore::Pgvector.new(model_class: NightlyEmptyDoc, dimension: 3) }
  end
end

...
RSpec.describe "Optional dependency absence", :nightly do
  describe "Phronomy::RAG::VectorStore::Pgvector without pgvector gem" do
    it "raises LoadError with a helpful message if pgvector is not installed" do
      script = <<~RUBY
        ...
        begin
          Phronomy::RAG::VectorStore::Pgvector.new(model_class: nil)
          puts "LOADED_OK"
        rescue LoadError => e
  ...
  describe "Phronomy::RAG::VectorStore::RedisSearch without redis gem" do
    it "raises LoadError with a helpful message if redis is not installed" do
      script = <<~RUBY
        ...
        begin
          Phronomy::RAG::VectorStore::RedisSearch.new(redis: nil, dimension: 3)
          puts "LOADED_OK"
        rescue LoadError => e
  ...
  describe "Phronomy core load without optional gems" do
    ...
    it "exposes the expected core constants" do
      ...
      expect(defined?(Phronomy::RAG::VectorStore::InMemory)).to eq("constant")

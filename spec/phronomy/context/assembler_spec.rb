...
RSpec.describe Phronomy::Context::Assembler do
  ...
  describe "#initialize" do
    it "stores the budget so that build uses it for token trimming" do
      budget = Phronomy::Context::TokenBudget.new(context_window: 1, max_output_tokens: 0)
  ...
  describe "#trim_messages_to_budget" do
    ...
    let(:budget) { Phronomy::Context::TokenBudget.new(context_window: 10, max_output_tokens: 0) }
    subject(:assembler) { described_class.new(budget: budget) }
    ...
    it "includes exactly the correct number of messages that fit within the budget" do
      tight = Phronomy::Context::TokenBudget.new(context_window: 3, max_output_tokens: 0)

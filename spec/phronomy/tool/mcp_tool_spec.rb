...
RSpec.describe Phronomy::Tool::McpTool do
  ...
  describe "StdioTransport startup_timeout (Issue #155)" do
    it "raises ToolError when stdout is not readable within startup_timeout" do
      stdin_dbl = instance_double(IO, closed?: false, puts: nil, close: nil)
      stdout_dbl = instance_double(IO, close: nil)
      stderr_dbl = instance_double(IO, read: nil, close: nil)
      wait_dbl = instance_double(Thread, join: nil)

      allow(Open3).to receive(:popen3).and_return([stdin_dbl, stdout_dbl, stderr_dbl, wait_dbl])
      ...
      expect { transport.send(:ensure_started!) }.to raise_error(
        Phronomy::ToolError, /did not start within/
      )
    end

    it "does not call gets on stdout when startup_timeout check succeeds" do
      stdin_dbl = instance_double(IO, closed?: false, puts: nil)
      stdout_dbl = instance_double(IO)
      stderr_dbl = instance_double(IO, read: nil)
      wait_dbl = instance_double(Thread, join: nil)

      allow(Open3).to receive(:popen3).and_return([stdin_dbl, stdout_dbl, stderr_dbl, wait_dbl])
      allow(IO).to receive(:select).and_return([[stdout_dbl]])
      allow(stdout_dbl).to receive(:gets)
      ...
      expect(stdout_dbl).not_to have_received(:gets)
    end

    it "skips IO.select entirely when startup_timeout is nil" do
      stdin_dbl = instance_double(IO, closed?: false, puts: nil)
      stdout_dbl = instance_double(IO)
      stderr_dbl = instance_double(IO, read: nil)
      wait_dbl = instance_double(Thread, join: nil)
      ...
      expect(IO).not_to have_received(:select)
      expect(stdout_dbl).not_to have_received(:gets)
    end
  end
end

...
RSpec.describe "Agent::Base blocking filter integration" do
  ...
  describe "#add_input_filter" do
    ...
    it "does not raise when input passes the check" do
      ...
      response = double("response", content: "ok", tool_call?: false, tokens: double(input: 5, output: 2, cache_read: 0, cache_write: 0, to_h: {"input" => 5, "output" => 2, "cached" => 0, "cache_creation" => 0}))
  ...
  describe "#add_output_filter" do
    it "raises FilterBlockError when output fails the check" do
      ...
      chat_double = double("Chat")
      response = double("response", content: "here is your SECRET key", tool_call?: false, tokens: double(input: 5, output: 10, cache_read: 0, cache_write: 0, to_h: {"input" => 5, "output" => 10, "cached" => 0, "cache_creation" => 0}))
      allow(RubyLLM).to receive(:chat).and_return(chat_double)
      allow(chat_double).to receive(:with_tools).and_return(chat_double)
      allow(chat_double).to receive(:with_instructions).and_return(chat_double)
      allow(chat_double).to receive(:with_temperature).and_return(chat_double)

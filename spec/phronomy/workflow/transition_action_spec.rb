  ...
  it "targets synthetic state_completed events at the owning fsm_session_id" do
    workflow = Phronomy::Workflow.define(context_class) do
      ...
      transition(
        ...
        to: :__finish__,
        action: ->(context, event) {
          owner_fsm_session_id = Phronomy::WorkflowExecutionRegistry.for(Phronomy::Runtime.instance.event_loop).workflow_admission_fsm_session_id(context.workflow_instance_id)
          context.merge(
            event_type: event.type,
            event_payload: event.payload,

...
RSpec.describe "Cooperative scheduler fairness (Issue #269)" do
  ...
  let(:loop_instance) { runtime.event_loop }
  ...
  describe "EventLoop lag metrics" do
    it "records last_lag_seconds after dispatching an event" do
      ...
      expect(loop_instance.last_lag_seconds).to be >= 0
      expect(loop_instance.max_lag_seconds).to be >= 0
      expect(loop_instance.average_lag_seconds).to be >= 0
    end

    it "max_lag_seconds is >= last_lag_seconds" do
      expect(loop_instance.max_lag_seconds).to be >=
        loop_instance.last_lag_seconds
    end
  end

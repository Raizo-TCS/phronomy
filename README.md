...
| **Cooperative scheduler yield points** — `Runtime#yield` (cooperative yield; calls `Thread.pass` on `ThreadScheduler`); `Runtime#yield_if_needed(every: N)` (thread-local counter, yields every N calls); CPU-bound detection when `blocking_detect_threshold_ms` is set (warns and increments `tasks_waiting_over_threshold` when a task runs longer than the threshold without yielding); `starvation_threshold_ms` configuration field (default: 50ms) | Beta |
